...
from .subsets import YouTubeDataset, NuScenesDataset, EveDataset
...
def dataset_mapping(subset_list: list, target_height: int, target_width: int, num_frames: int, h5_file = None):
    datasets = list()
    for subset_name in subset_list:
        ...
        elif subset_name == "EVE":
            datasets.append(
                EveDataset(h5_file, 6, target_height=target_height, target_width=target_width, num_frames=num_frames)
            )
        else:
...
class MultiSourceSamplerDataset(Dataset):
    def __init__(self, subsets, probs, samples_per_epoch=1000, target_height=320, target_width=576, num_frames=25, h5_file = None):
        self.subsets = dataset_mapping(subsets, target_height, target_width, num_frames, h5_file)
...
class Sampler(LightningDataModule):
    def __init__(self, batch_size, num_workers=0, prefetch_factor=2, shuffle=True, subsets=None, probs=None,
                 samples_per_epoch=None, target_height=320, target_width=576, num_frames=25, h5_file = None):
        super().__init__()
        ...
        self.train_dataset = MultiSourceSamplerDataset(
            ...
            target_height=target_height, target_width=target_width, num_frames=num_frames, h5_file = h5_file
        )
